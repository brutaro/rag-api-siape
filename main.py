...
async def get_context_and_sources(original_query: str, top_k_initial: int, top_k_final: int) -> (str, List[str]):
    ...
    final_context_chunks = [match['metadata'].get('text', '') for score, match in top_results]
    final_context = "\n\n---\n\n".join(final_context_chunks)
    sources = list(set([match['metadata'].get('source', 'Fonte desconhecida') for score, match in top_results]))
    
    return final_context, sources

async def stream_final_answer(final_prompt: str) -> AsyncGenerator[str, None]:
    """Gera a resposta final da OpenAI em modo streaming."""
    try:
        ...
        logger.error(f"Erro durante o streaming da OpenAI: {e}")
        yield "Desculpe, ocorreu um erro ao gerar a resposta final."
...
async def process_query_streaming(request: QueryRequest):
    ...
    async def response_generator():
        try:
            final_context, sources = await get_context_and_sources(request.question, request.top_k_initial, request.top_k_final)

            sources_payload = json.dumps({"sources": sources})
            yield f"{sources_payload}\n---\n"

            if not final_context:
                yield "Não encontrei informações relevantes para sua pergunta nos documentos disponíveis."
                return

            # <<<<< PROMPT COMPLETO E CORRETO INSERIDO AQUI >>>>>
            final_prompt = f"""
